...
void print_usage(const char* program) {
    std::cout << "Usage:\n"
              ...
              << "This target uses threadIdx.x with launch config <<<1, N>>>.\n"
              << "N must be less than or equal to the device maxThreadsPerBlock.\n";
}

__global__ void add_by_thread(const int* a, const int* b, int* c, int n) {
    int i = threadIdx.x;
    if (i < n) {
        c[i] = a[i] + b[i];
    }
...
float benchmark_kernel(const int* d_a, const int* d_b, int* d_c, int n, int benchmark_runs) {
    for (int i = 0; i < WARMUP_RUNS; ++i) {
        add_by_thread<<<1, n>>>(d_a, d_b, d_c, n);
    }
...
int main(int argc, char** argv) {
    ...
    if (n > prop.maxThreadsPerBlock) {
        std::cerr << "add_by_thread launches <<<1, N>>> and requires N <= maxThreadsPerBlock.\n"
                  << "Current N = " << n
    ...
    std::string launch_config = "<<<1, " + std::to_string(n) + ">>>";
    ...
    std::cout << std::left << std::setw(24) << "Thread-based add" << std::setw(20)
              << launch_config << std::setw(18) << "threadIdx.x" << std::setw(18)
