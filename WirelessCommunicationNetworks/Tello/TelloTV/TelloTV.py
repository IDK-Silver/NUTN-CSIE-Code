...
class FrontEnd(object):
    ...
    def get_battery_safe(self):
        """Safely get battery level as integer"""
        try:
            battery_str = str(self.tello.get_battery())
            # Remove any whitespace and newlines
            battery_str = battery_str.strip()
            # Try to convert to int
            return int(battery_str)
        except (ValueError, AttributeError):
            # If conversion fails, return -1 as error indicator
            return -1
    ...
    def run(self):
        ...
        # Wait for camera to initialize and start sending valid frames
        time.sleep(2.5)
        ...
        battery = self.get_battery_safe()
        if battery >= 0:
            print(f"Battery: {battery}%")
            if battery < 30:
                print(f"WARNING: Battery level is low ({battery}%). Recommend charging before flight.")
        else:
            print("Warning: Could not read battery level")
            battery = 0  # Set to 0 for display purposes
        ...
        while not should_stop:
            ...
            # Skip invalid frames during camera initialization
            if frame_read.frame is None or frame_read.frame.size == 0:
                continue
            ...
            if k == ord('t'):
                if not args.debug:
                    ...
                    battery = self.get_battery_safe()
                    if battery >= 0:
                        print(f"Battery after takeoff: {battery}%")
            ...
            # Draw battery level
            batteryCol = (0,255,0) if battery > 50 else (0,255,255) if battery > 30 else (0,0,255)
            cv2.putText(frameRet,f"Battery: {battery}%",(32,32),cv2.FONT_HERSHEY_SIMPLEX,1,batteryCol,2)

            # Display the resulting frame
            cv2.imshow(f'Tello Tracking...',frameRet)

        # On exit, print the battery
        final_battery = self.get_battery_safe()
        if final_battery >= 0:
            print(f"Final battery level: {final_battery}%")
        else:
            print("Could not read final battery level")
